...
export const config = {
  ...
  // The server always binds 127.0.0.1. Reaching it from outside is Cloudflare
  // Tunnel's job: `cloudflared` runs on this machine and dials out, so the port
  // is never exposed on the LAN or the router. There is no bind-address knob on
  // purpose — opening the port directly is the mistake this design prevents.
  //
  // With REQUIRE_AUTH off (default) the dashboard trusts anything that reaches
  // it, which is correct for localhost. Turn it on before the tunnel goes up.
  requireAuth: bool('REQUIRE_AUTH', false),
  // From the Cloudflare Access application — see docs/DEPLOY.md step 4.
