...
// The one seam between the core and the CRM. Removing the CRM is deleting
// lib/crm/ and this import, plus the two references to it below.
import { CRM_SCHEMA, CRM_MIGRATIONS } from './crm/schema.js';
...
// The CRM's tables and migrations (003-007) live in lib/crm/. Concatenating
// here reproduces the original single-list order exactly, so ids already
// recorded in schema_migrations are not re-run.
const FULL_SCHEMA = SCHEMA + CRM_SCHEMA;
const MIGRATIONS = [...CORE_MIGRATIONS, ...CRM_MIGRATIONS];
...
// CRM queries (flips, parts) now live in lib/crm/db.js. Only the schema seam
// at the top of this file connects the two halves.
